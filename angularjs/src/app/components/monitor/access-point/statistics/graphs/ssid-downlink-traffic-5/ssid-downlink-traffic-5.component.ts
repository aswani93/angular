...
@Component({
  selector: 'app-ssid-downlink-traffic-5',
  templateUrl: './ssid-downlink-traffic-5.component.html',
  styleUrls: ['./ssid-downlink-traffic-5.component.css']
})
export class SsidDownlinkTraffic5Component implements OnInit {
            ...
            load: function () { 
             ...
             // if (t.liveFlag == true) {
                ...
                t.timerVar = setInterval(function () {
                  t._service.getWeb('statistics/down-link-traffic-vap-5/?group='+t.groupid+'&scale='+timeFlag+'&vap=' + t.selectedMac+'&time='+t.timestamp+'', '', '').then(_data => {
 ...
 //this.drawGraph(this.timeStamp);
  ...
  loadData(param, status) {console.log(this.groupid,'loaddate');
   // this.spinnerService.show();
      this._service.getWeb('statistics/down-link-traffic-vap-5/?group='+this.groupid+'&scale='+param+'&vap=' + this.selectedMac + '', '', '').then(_data => {
        if (_data.status!=0) {
