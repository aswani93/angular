...
@Component({
  selector: 'app-ssid-downlink-traffic',
  templateUrl: './ssid-downlink-traffic.component.html',
  styleUrls: ['./ssid-downlink-traffic.component.css']
})
export class SsidDownlinkTrafficComponent implements OnInit {
              ...
              load: function () { 
                ...
                //if (t.liveFlag == true) {
                  ...
                  t.timerVar = setInterval(function () {
                    t._service.getWeb('statistics/down-link-traffic-vap-24/?group='+t.groupid+'&scale='+timeFlag+'&vap=' + t.selectedMac+'&time='+t.timestamp+'', '', '').then(_data => {
   ...
   //this.drawGraph(this.timeStamp);
    ...
    loadData(param, status) {console.log('load');
     // this.spinnerService.show();
        this._service.getWeb('statistics/down-link-traffic-vap-24/?group='+this.groupid+'&scale='+param+'&vap=' + this.selectedMac + '', '', '').then(_data => {
          if (_data.status!=0) {
      ...
      drawGraph(obj) {console.log('draw graph')
        this.timeStamp = obj;
        if (obj == 'live') {
          this.liveFlag = true;
        } else {
